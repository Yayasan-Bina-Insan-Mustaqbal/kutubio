...
class ImportBorrowersFromGoogleSheet extends Command
{
    ...
    public function handle(): int
    {
        ...
        $academicYear = $this->academicYear($this->argument('academicYear'));
        ...
        foreach ($rows as $index => $values) {
            ...
            $status = $name === '' || $grade === null || $section === null ? 'invalid' : ($candidates->count() === 1 ? 'matched' : ($candidates->count() === 0 ? 'new' : 'needs_review'));
            $counts[$status]++;
    ...
    private function commitRows(BorrowerImport $import): void
    {
        foreach ($import->load('rows')->rows as $row) {
            ...
            $data = $row->raw_data;
            $attendanceNumber = (int) ($data['no_urut'] ?? $row->row_number);
            $identifier = $row->matched_borrower_id
                ? null
                : 'student-'.$this->academicYearPrefix($import->academic_year).$this->classSectionCode($row->grade, $row->section).str_pad((string) $attendanceNumber, 2, '0', STR_PAD_LEFT);
    ...
    {
        return preg_match('/^(\d+)([A-Z])$/', $class, $matches) ? [(int) $matches[1], $matches[2]] : [null, null];
    }
}
